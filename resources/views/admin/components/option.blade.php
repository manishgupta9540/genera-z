<div class="row py-1">
    <label for="answer_{{$id}}" class="m-0 col-sm-1 d-flex justify-content-center align-items-center">
        <input type="radio" class="answerRadio" name="data[{{ $question_id }}][answer]" value="{{$id}}" id="answer_{{$id}}" {{isset($question->answer) && isset($question->answer->option_id) && $question->answer->option_id == $id ? 'checked' : '' }}>
    </label>
    <div class="col-sm-10">
        @include('components.input', [
            'id' => 'data_'.$question_id.'_options_'.$id.'_content',
            'name' => 'data['.$question_id.'][options]['.$id.'][content]',
            'label' => '',
            'type' => 'text',
            'mandatory' => true,
            'class' => '',
